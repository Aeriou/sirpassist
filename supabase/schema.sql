-- SiprAssist — copie cloud (exécuter une fois : Supabase → SQL Editor → New query → coller → Run)
-- Source : src/lib/supabase-schema.ts (constante SUPABASE_SCHEMA_SQL).
-- Ce backend sera remplacé par des tables Neon en Phase 3 ; il n'existe ici que
-- pour que la version de référence (branche main) fonctionne à l'identique.
